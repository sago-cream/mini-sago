  ...
  try {
    ...
    recorder.onstop = async () => {
      ...
      try {
        ...
        const capture = snapshot.events.findLast(
          (event) =>
            event.sessionId === sessionId && event.type === "utterance.queued",
        );
        if (!capture) throw new Error("Captured audio is unavailable");
        $("capture-audio").src =
          `/api/voice-debug/audio?session=${encodeURIComponent(sessionId)}&turn=${encodeURIComponent(capture.turnId)}`;
